...
# Programming spring 2020). Check that the required files exist and can be built
...
data_dir=/proj/g2020012/nobackup/matmul_indata
output_file="output_to_remove.txt"
commands=(
	"./matmul ${data_dir}/input4.txt ${output_file}"
)
expected_output=(
	"^\s*280.000000\s+290.000000\s+300.000000\s+310.000000(\s+[0-9]*\.[0-9]{6}){8}\s+1528.000000\s+1586.000000\s+1644.000000\s+1702.000000\s*$"
)
